...
// TestOverlayForProfileReplacesTheActiveSelection 构建期 overlay 只替换选点文件，键值为绝对
// 路径，内容指向请求的形态；未知形态在生成前就被 registry 拦下。
func TestOverlayForProfileReplacesTheActiveSelection(t *testing.T) {
	root := t.TempDir()
	overlay, err := overlayForProfile(root, "minimal")
	require.NoError(t, err)
	require.Len(t, overlay, 1)
	content, ok := overlay[filepath.Join(root, "internal", "profiles", "active", "assembly.go")]
	require.True(t, ok)
	assert.Contains(t, string(content), `"jimu/internal/profiles/minimal"`)
	assert.Contains(t, string(content), "Code generated by tools/profileoverlay")

	_, err = overlayForProfile(root, "ghost")
	require.ErrorContains(t, err, `unknown profile "ghost"`)
}

// TestWriteOverlayPointsTheConfigAtTheGeneratedFile go build -overlay 读的是 JSON 文件：
// 它必须把仓库里的选点文件映射到临时目录里的生成物。
func TestWriteOverlayPointsTheConfigAtTheGeneratedFile(t *testing.T) {
	root, dir := t.TempDir(), t.TempDir()
	path, err := writeOverlay(root, "enterprise", dir)
	require.NoError(t, err)
	assert.Equal(t, filepath.Join(dir, "overlay.json"), path)
