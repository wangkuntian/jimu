...
func newTestDB(t *testing.T) *gorm.DB {
	...
	require.NoError(t, db.AutoMigrate(&domain.APIKey{}))
	return db
}

// createKey 插入 API Key。用 map 避免 gorm 零值省略（sqlite default:true 会掩盖显式 false）
func createKey(t *testing.T, db *gorm.DB, key *domain.APIKey) *domain.APIKey {
	err := db.Model(&domain.APIKey{}).Create(map[string]interface{}{
...
func TestDBAPIKeyStore_UpdateLastUsed(t *testing.T) {
	db := newTestDB(t)
	row := createKey(t, db, &domain.APIKey{Name: "t", KeyHash: "h", Enabled: true})

	store := NewDBAPIKeyStore(db)
	now := time.Now()
	require.NoError(t, store.UpdateLastUsed(context.Background(), row.ID, now))

	var got domain.APIKey
...
func TestVerifyWithDBStore(t *testing.T) {
	...
	require.NoError(t, db.Create(&domain.APIKey{
		Name:      "t",
