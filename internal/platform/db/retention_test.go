...
func TestRetentionServiceDeletesOnlyExpiredRows(t *testing.T) {
	...
	svc := NewRetentionService(db, config.RetentionConfig{BatchSize: 100, AuditLogDays: 30})
...
func TestRetentionServiceDeletesInBatches(t *testing.T) {
	...
	svc := NewRetentionService(db, config.RetentionConfig{BatchSize: 2, AuditLogDays: 30})
	results, err := svc.Run(ctx)
	require.NoError(t, err)
	require.Len(t, results, 1)
...
func TestDefaultRetentionRulesSkipZeroDays(t *testing.T) {
	rules := DefaultRetentionRules(config.RetentionConfig{
		AuditLogDays:    180,
		OutboxEventDays: 7,
	})
