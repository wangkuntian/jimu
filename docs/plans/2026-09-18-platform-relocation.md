...
- **不留残余**：Go / 脚本 / 工作流 / Makefile / README / AGENTS.md / specs / configs 中不得再有 `internal/platform` 引用。四处有意的历史例外：P0 计划、P1-A 计划与本计划（它们的主题就是搬迁）、设计文档 §3.5.3/§3.6 的归位表（属改造前现状，需加历史注记）、以及已发布的历史版本日志
...
- 历史例外：`docs/releases/v0.2.0.md`（已发布记录，不重写）
...
**Files:**
- Modify: `Makefile:288`、`scripts/bench_ci.sh:13`、`scripts/govulncheck.sh:5`、`.github/workflows/ci.yml:178`、`.github/workflows/release.yml:89`、`AGENTS.md:41-43`、`README.md`（15 行，含目录树）、`specs/**`（5 文件 15 行）、`docs/releases/v0.3.0.md`（1 行）、`docs/design/2026-09-18-capability-plugins-design.md`（§3.6 的 tenant 归属措辞 + §3.5.3 历史注记）
...
```bash
# 3a) 加宽模式的残留：只允许命中四处历史例外（P0 计划 / P1-A 计划 / 本计划 / 设计文档）
...
```bash
git add README.md AGENTS.md Makefile scripts/ .github/ specs/ docs/ && git commit -m "docs(platform): update path references for the platform relocation"
```
