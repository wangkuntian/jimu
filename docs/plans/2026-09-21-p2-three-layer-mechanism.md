...
- **已完成（两道保险）**：`make check-capabilities` 由 1 条断言扩到 **5 条** —— ① `Owns`↔迁移归属（既有）② 驱动可用集↔目录存在 ③ 核心包生产闭包零驱动、零重型依赖 ④ 形态选中 == 形态生产 import 闭包（集合比较）⑤ 形态生产代码只 import 已声明的驱动包（抓「新增驱动目录 + blank import 却忘声明」）；`make compose-report` 新增「重型依赖」列。两者**仍不接入** `make ci`/`release-check`（P2.8 收口）。
