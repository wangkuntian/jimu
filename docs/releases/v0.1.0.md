...
- 移除多租户抽象，明确单租户 RBAC 定位（v0.2.0 起以全新设计恢复，见 v0.2.0）
...
- `COMPOSE_ENV` 环境变量在 `make release-check` 中的集成尚未实现（规范提及 `make release-check COMPOSE_ENV=.env.example`，当前 release-check target 为 go 层门禁，不消费该变量）；compose 集成验证作为后续项
